    ...
    <el-card>
      ...
      <div class="table-container">
        <el-table row-key="id" size="medium" default-expand-all :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" border :data="table.data">
          ...
          <el-table-column label="负责人" align="center" header-align="center" prop="leader" :show-overflow-tooltip="true" />
          <el-table-column label="联系电话" align="center" header-align="center" prop="phone" :show-overflow-tooltip="true" />
          ...
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label="联系电话" prop="phone">
                <el-input v-model="form.data.phone" placeholder="请输入联系电话" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
<script>
import { deptPage, deptAdd, deptUpdate, deptDel } from '@/api/sys/dept'
...
export default {
  ...
  methods: {
    ...
    tablePage() {
      ...
      deptPage(this.search).then(response => {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if (this.form.data.id === undefined) {
            deptAdd(this.form.data).then((res) => {
              this.form.dialog = false
              this.tablePage()
              this.$refs['form'].resetFields()
            })
          } else {
            deptUpdate(this.form.data).then((res) => {
              this.form.dialog = false
              this.tablePage()
            })
          }
    ...
    tableEnabledChange(row) {
      ...
      this.$confirm(`确认要 ${text} ${row.name} 吗?`, '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(function() {
        return deptUpdate(Object.assign({}, row))
      }).then(() => {
